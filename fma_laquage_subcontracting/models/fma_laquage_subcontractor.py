...
class FmaLaquageSubcontractor(models.Model):
    _name = 'fma.laquage.subcontractor'
    _description = 'Sous-traitant laquage FMA/F2M'
    ...
    product_id = fields.Many2one(
        'product.product',
        string='Article achat laquage',
        domain=[('purchase_ok', '=', True)],
        help="Article de service utilisé pour créer la demande d'achat / commande fournisseur.",
    )
    currency_id = fields.Many2one(related='company_id.currency_id', readonly=True)
    price_unit = fields.Monetary(string='Prix unitaire', currency_field='currency_id')
    qty_field_name = fields.Char(
        string='Champ quantité/m² sur OF',
        help="Optionnel. Nom technique d'un champ OF contenant la surface/quantité à acheter. Si vide, Odoo utilisera la quantité de l'OF.",
    )
    slot_ids = fields.One2many('fma.laquage.slot', 'subcontractor_id', string='Créneaux')


class FmaLaquageSlot(models.Model):
    _name = 'fma.laquage.slot'
    _description = 'Créneau laquage sous-traitant'
    _order = 'subcontractor_id, departure_weekday, return_week_offset, return_weekday'

    subcontractor_id = fields.Many2one('fma.laquage.subcontractor', required=True, ondelete='cascade')
    ...
    @api.depends('subcontractor_id.name', 'departure_weekday', 'return_weekday', 'return_week_offset')
    def _compute_name(self):
        ...
        for rec in self:
            ...
            rec.name = '%s : %s → %s%s' % (rec.subcontractor_id.name or '', dep, ret, suffix)
