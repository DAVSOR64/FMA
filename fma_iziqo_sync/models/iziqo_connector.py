...
class IziqoConnector(models.AbstractModel):
    ...
    def _iziqo_resources(self):
        """Modeles synchronisables : (modele, libelle, URL, testable en GET).

        Sert au test de connexion, qui n'interroge que les collections
        exposant un GET.
        """
        resources = []
        for model_name, label in self.env["iziqo.sync.job"]._selection_res_model():
            ...
            if model is None:
                continue
            resources.append(
                (model_name, label, model._iziqo_url(), model._iziqo_supports_get)
            )
        return resources
