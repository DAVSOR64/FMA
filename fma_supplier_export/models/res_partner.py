...
class ResPartner(models.Model):
    ...
    def _log_file_for_each_partner(self, partners, file):
        ...
        for partner in partners:
            ...
            partner.message_post(
                body=Markup(
                    _(
                        "Supplier Details files created: <a href='%s' target='_blank'>%s</a>"
                    )
                )
    ...
    def cron_generate_generate_supplier_files(self):
        ...
        try:
            ...
            file_name = (
                f"Supplier_Details_{fields.Datetime.now().strftime('%Y-%m-%d')}.txt"
            )
    ...
    def cron_send_supplier_file_to_sftp_server(self):
        ...
        for attachment in attachments:
            try:
                ...
                _logger.error(
                    f"Failed to sync supplier file {attachment.name}.txt to SFTP server: {e}"
                )
