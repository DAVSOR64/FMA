...
class ResPartner(models.Model):
    ...
    def write(self, vals):
        if not vals.get('is_included_in_supplier_export_file'):
            vals['is_included_in_supplier_export_file'] = False
    ...
    def _get_file_content(self, partners):
        ...
        for partner in partners:
            line = [
                    'PCF',
                    'I',
                    str(partner.property_account_payable_id or '').ljust(9),  # Code tiers
    ...
    def _log_file_for_each_partner(self, partners, file):
        ...
        for partner in partners:
            ...
            partner.is_included_in_supplier_export_file = True

    def cron_generate_generate_csupplier_files(self):
        """Cron to generate supplier details .txt file."""
        partners = self.search([('is_included_in_supplier_export_file', '=', False)])
        try:
            file_content = self._get_file_content(partners)
            file_name = f"supplier_Details_{fields.Datetime.now().strftime('%Y-%m-%d')}.txt"
            ...
            _logger.exception("Failed to create supplier file for %s: %s", self.name, e)

    def cron_send_supplier_file_to_sftp_server(self):
        """Sync the unsynced supplier files to SFTP server."""
        ...
        sftp_server_host = get_param('fma_supplier_export.sftp_server_host')
        sftp_server_username = get_param('fma_supplier_export.sftp_server_username')
        sftp_server_password = get_param('fma_supplier_export.sftp_server_password')
        sftp_server_file_path = get_param('fma_supplier_export.sftp_server_file_path')
        ...
        for attachment in attachments:
            try:
                ...
                _logger.error(f"Failed to sync supplier file {attachment.name}.txt to SFTP server: {e}")

    def _sync_file(self, attachment, sftp_server_host, sftp_server_username, sftp_server_password, sftp_server_file_path):
        """Upload the supplier file to the SFTP server."""
