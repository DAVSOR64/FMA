...
class AccountMove(models.Model):
    ...
    show_text_block = fields.Boolean(
        string="Show Text Block", compute="_compute_show_text_block"
    )
    inv_show_affacturage = fields.Boolean(
        string="Show Affacturage", compute="_compute_show_affacturage"
    )
    ...
    def create(self, vals):
        if isinstance(vals, list):
            for val in vals:
                if "invoice_origin" in val:
                    sale_order = self.env["sale.order"].search(
                        [("name", "=", val["invoice_origin"])], limit=1
                    )
                    if sale_order:
            ...
            if "invoice_origin" in vals:
                sale_order = self.env["sale.order"].search(
                    [("name", "=", vals["invoice_origin"])], limit=1
                )
                if sale_order:
