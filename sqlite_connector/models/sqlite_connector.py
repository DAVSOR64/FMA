...
class SqliteConnector(models.Model):
    ...
    state = fields.Selection(
        [("new", "New"), ("done", "Exported"), ("error", "Errors")],
        string="Status",
        readonly=True,
        default="new",
    )
    ...
    def export_data_from_db(self):
        ...
        temp_file = tempfile.NamedTemporaryFile(
            "wb", suffix=".sqlite", prefix="edi.mx.tmp."
        )
        ...
        article_data = cursor.execute(
            "select ArticleCode, PriceGross, PUSize, Units_Unit, ArticleCode_Supplier, ArticleCode_BaseNumber,ColorInfoInternal, Color, ArticleCode_Number, Discount from Articles"
        )
        ...
        for row in profile_data:
            ...
            if fournisseur == "SAPA":
                ...
                if RefLogikal.startswith("X"):
                    ...
                    RefLogikal = "S" + RefLogikal
            _logger.warning(
                "**********AVANT Profile pour MAJ********* %s " % RefLogikal
            )
            if fournisseur == "WICONA":
                refart = "WIC" + " " + row[2][1:]
                Length = row[9]
                if RefLogikal.startswith("X"):
                    ...
                    if int(str(LengthLogikal)) < 7:
                        ...
                        RefLogikal = "W" + RefLogikal
                _logger.warning(
                    "**********APRES Profile pour MAJ********* %s " % RefLogikal
                )
            if fournisseur == "JANSEN" or fournisseur == "Jansen":
                refart = "JAN" + " " + row[2]
            refart = refart.replace("RYN", "REY")
        ...
        for profile in profiles:
            _logger.warning(
                "**********Profile dans la MAJ********* %s " % profile["article"]
            )
            product = self.env["product.product"].search(
                [("default_code", "=", profile["article"])]
            )
            if product:
                _logger.warning(
                    "**********Profile dans la MAJ pour ref logikal********* %s "
                    % profile["RefLogikal"]
                )
                product = product[0]
        ...
        resultp = cursor.execute(
            "select Projects.Name, Projects.OfferNo, PersonInCharge from Projects"
        )
        ...
        user_id = res_users.filtered(
            lambda p: p.name == re.sub(" +", " ", PersonBE.strip())
        )
        if user_id:
            user_id = user_id.id
        else:
        ...
        account_analytic_id = account_analytics.filtered(
            lambda a: a.name.strip() in projet.strip()
        )
        ...
        resultBP = cursor.execute(
            "select subNode, FieldName, SValue from REPORTVARIABLES"
        )
        for row in resultBP:
            _logger.warning(
                "*****************************UserVars**************** %s " % row[0]
        ...
        if BP != "BPA":
            ...
            for row in resultsm:
                ...
                categ = product_categories.filtered(
                    lambda c: c.x_studio_logical_map == categorie
                )
                if not categ:
                    self.log_request(
                        "Unable to find product category.", categorie, "Elevations data"
                    )
                if row[1] != "ECO-CONTRIBUTION" and not self.env[
                    "product.product"
                ].search([("default_code", "=", refint)]):
        ...
        if warehouse:
            stock_picking_type_id = stock_picking_type.filtered(
                lambda p: p.name == operation.strip()
                and p.warehouse_id.id == warehouse.id
            )
            if stock_picking_type_id:
                stock_picking_type_id = stock_picking_type_id[0].id
            else:
                self.log_request(
                    "Unable to find stock picking type.",
                    operation.strip(),
                    "Project Data",
                )
        else:
        ...
        def creation_commande(
            Commande, refinterne, unstk, fournisseur, prix, delai, UV, Qte
        ):
            trouve = False
        ...
        for row in resart:
            ...
            if fournisseur != "HUD":
                ...
                if not self.env["product.product"].search(
                    ...
                    creation_commande(
                        Commande, refart, unit, fournisseur, prix, delai, UV, Qte
                    )
        ...
        for ligne in Article:
            ...
            resultat = res_partners.filtered(
                lambda p: p.x_studio_ref_logikal
                and p.x_studio_ref_logikal.upper() == fournisseur
            )
            if resultat:
                idfrs = resultat[0].id
            ...
            if not self.env["product.product"].search(
                ...
                vals = {
                    ...
                    "route_ids": [
                        Command.link(self.env.ref("stock.route_warehouse0_mto").id),
                        Command.link(
                            self.env.ref("__export__.stock_route_54_b165c5dc").id
                        ),
                    ],
        ...
        if BP != "BPA-BPE":
            ...
            for row in resultpf:
                ...
                if not self.env["product.product"].search(
                    ...
                    creation_commande(
                        Commande, refart, unit, fournisseur, prixB, delai, UV, Qte
                    )
            ...
            for ligne in Article:
                ...
                if resultat:
                    ...
                    self.log_request(
                        "Unable to find customer (x_studio_ref_logikal)",
                        fournisseur,
                        "Articles Data",
                    )
                ...
                if idfrs != "":
                    ...
                    if not self.env["product.product"].search(
                        ...
                        product = self.env["product.product"].create(vals)
                        message = (
                            _("Product has been Created: ") + product._get_html_link()
                        )
                        self.message_post(body=message)
                        self.env.cr.commit()
                        # created nomenclature
        ...
        if BP != "BPA":
            ...
            for row in resultg:
                ...
                if res_partner:
                    ...
                    frsnomf = res_partner[
                        0
                    ].name  # ou une autre logique selon ton besoin
                else:
                    self.log_request(
                        "Unable to find supplier with LK Supplier ID",
                        str(Frsid),
                        "Glass Data",
                    )
                ...
                if pb:
                    ...
                    def join_positions(data, i_start, i_end):
                        return " # ".join(
                            [f"{fmt(pbs[i_start])}/{fmt(pbs[i_end])}" for pbs in data]
                        )
            ...
            for ligne in Glass:
                ...
                if fournisseur != ligne[0] or info_livraison != ligne[1]:
                    x_affaire = self.env["x_affaire"].search(
                        [("x_name", "ilike", projet)], limit=1
                    )
                ...
                if vitrage != (
                    ...
                    categ_id = self.env.ref(
                        "__export__.product_category_23_31345211"
                    ).id
        ...
        if BP != "BPA":
            for row in resultp:
                ...
                sale_order = self.env["sale.order"].search(
                    [("name", "=", proj), ("state", "not in", ["done", "cancel"])],
                    limit=1,
                )
                ana_acc = self.env["account.analytic.account"].search(
                    [("name", "ilike", projet)], limit=1
                )
                ...
                if sale_order:
                    if so_data.get(sale_order.id, 0) == 0 and pro:
                        so_data[sale_order.id] = {
                            ...
                            "partner_shipping_id": part.id
                            if part
                            else sale_order.partner_shipping_id.id,
                            "partner_invoice_id": part.id
                            if part
                            else sale_order.partner_invoice_id.id,
                        ...
                        if (
                            pro
                            and so_data[sale_order.id]
                            and so_data[sale_order.id].get("order_line")
                        ):
                            so_data[sale_order.id].get("order_line").append(
                                Command.create(
                                    {
                                        "product_id": pro.id,
                                        "price_unit": price,
            ...
            sale_order = self.env["sale.order"].search(
                [("name", "=", proj), ("state", "not in", ["done", "cancel"])], limit=1
            )
        ...
        for row in Nomenclature:
            ...
            if datanom1[1] != "":
                pro_t = self.env["product.product"].search(
                    [("default_code", "=", datanom1[1])], limit=1
                )
                if not pro_t:
                    self.log_request(
                        "Unable to find product", datanom1[1], "Nomenclatures Creation"
                    )
                else:
                ...
                if nomenclatures_data and pro:
                    nomenclatures_data[0].get("bom_line_ids").append(
                        Command.create(
                            {
                                "product_id": pro[0].id,
                                "product_qty": Qte,
        ...
        resuOpe = cursor.execute(
            "SELECT LabourTimes.TotalMinutes, LabourTimes.WhatName, LabourTimes.Name, LabourTimes.LabourTimeId FROM LabourTimes order by CAST(LabourTimes.LabourTimeId AS INTEGER)"
        ).fetchall()
        ...
        for rowOpe in resuOpe:
            ...
            if rowOpe[2] is not None and rowOpe[2] != "":
                if (
                    rowOpe[2].strip() == "Parcloses ALU"
                    or rowOpe[2].strip() == "Emballage"
                ) and eticom == "F2M":
        ...
        for ope, data in aggregated_data.items():
            ...
            if nomenclatures_data:
                nomenclatures_data[0]["operation_ids"].append(
                    Command.create(operation_data)
                )
