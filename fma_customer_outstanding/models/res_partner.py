...
class ResPartner(models.Model):
    ...
    def cron_update_outstandings(self):
        ...
        try:
            ...
            try:
                ...
                transport.connect(
                    username=ftp_server_username, password=ftp_server_password
                )
                ...
                try:
                    ...
                    _logger.error(
                        "SFTP path not found or not accessible: %s",
                        ftp_server_file_path,
                    )
                    return
                ...
                _logger.error(
                    "Error while connecting or retrieving file from SFTP: %s",
                    sftp_error,
                )
                return
            finally:
            ...
            try:
                ...
                _logger.info(
                    "Customer outstandings successfully updated from %s.", filename
                )
            finally:
                try:
            ...
            _logger.error(
                "Failed to download customer file %s from SFTP server: %s", filename, e
            )
    ...
    def _update_customer_outstandings(self, file_content):
        ...
        if champ not in self._fields:
            _logger.warning(
                "Champ %s inexistant sur res.partner. Champs dispo (extrait): %s",
                champ,
                list(self._fields.keys())[:50],
            )
