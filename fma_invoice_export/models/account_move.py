...
class AccountMove(models.Model):
    ...
    def action_create_journal_items_file(self):
        ...
        for move in self.filtered(
            lambda move: not move.is_txt_created and move.state == "posted"
        ):
            try:
    ...
    def _get_file_content(self, journal_items, move):
        ...
        journal = "VTE"
        _logger.warning(
            "**********dans la fonction pour l export facture vente********* "
        )
        ...
        for account_code, items_grouped_by_account in groupby(
            ...
            if account_code:
                ...
                if move.name.startswith(("FC", "AVV")) and len(move.name) >= 6:
                    prefix = move.name[:2]
                    year = move.name[2:6]
                    name_invoice = move.name.replace(
                        f"{prefix}{year}", f"{prefix}{year[2:]}", 1
                    )
                ...
                # silencieusement le contenu de toutes les lignes.
                #
                # Le referentiel ne sert que de repli, pour les factures ou le
