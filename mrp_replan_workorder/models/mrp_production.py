    ...
    def button_plan(self):
        ...
        for production in self:
            ...
            if not date_debut_of:
                _logger.warning(
                    "Pas de date de début sur l'OF %s, on saute", production.name
                )
                continue

            # Trier les workorders par ordre des opérations
            workorders = sorted(
                production.workorder_ids, key=lambda wo: wo.operation_id.sequence
            )
            ...
            for workorder in workorders:
                ...
                if workorder != workorders[0]:
                    delay_obj = self.env["x_delai_entre_operatio"].search(
                        [
                            (
                                "x_studio_poste_de_travail_deb",
                                "=",
                                previous_workorder.workcenter_id.id,
                            ),
                            (
                                "x_studio_poste_de_travail_fin",
                                "=",
                    ...
                    delay_minutes = (
                        delay_obj.x_studio_dlai_entre_oprations if delay_obj else 0
                    )
                    _logger.info(
                        "Délai entre %s → %s = %s min",
                        previous_workorder.workcenter_id.name,
                        workorder.workcenter_id.name,
                        delay_minutes,
                    )
                    _logger.info(
                        "Opération %s : Start = %s ", workorder.name, previous_end_date
                    )

                # Calcul de la date de début
                workorder.date_start = previous_end_date + timedelta(
                    minutes=delay_minutes
                )
                _logger.info(
                    "Opération %s : Start = %s ", workorder.name, workorder.date_start
                )

                # Calcul de la date de fin en fonction de la durée attendue
                duration = workorder.duration_expected or 0.0
                workorder.date_finished = workorder.date_start + timedelta(
                    minutes=duration
                )
                _logger.info(
                    "Opération %s : Start = %s ",
