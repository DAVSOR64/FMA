...
class MrpProduction(models.Model):
    ...
    def _get_replan_target_end_dt(self):
        self.ensure_one()
        so = False
        if hasattr(self, 'x_studio_mtn_mrp_sale_order') and self.x_studio_mtn_mrp_sale_order:
            so = self.x_studio_mtn_mrp_sale_order
        ...
        target_dt = self.macro_forced_end or self.date_finished
        if so and (so.commitment_date or so.expected_date):
            target_dt = fields.Datetime.to_datetime(so.commitment_date or so.expected_date)
        return fields.Datetime.to_datetime(target_dt) if target_dt else fields.Datetime.now()

    def _collect_related_purchase_orders(self):
        self.ensure_one()
        pos = self.env['purchase.order']
        moves = (self.move_raw_ids | self.move_finished_ids)
        for move in moves:
            if getattr(move, 'purchase_line_id', False):
                pos |= move.purchase_line_id.order_id
            if move.move_orig_ids:
                for om in move.move_orig_ids:
                    if getattr(om, 'purchase_line_id', False):
                        pos |= om.purchase_line_id.order_id
        return pos.sorted(lambda p: p.date_planned or fields.Datetime.now())
    ...
    def _build_replan_preview_payload(self):
        ...
        if not workorders:
            return {'production_id': self.id, 'lines': [], 'mo_start': False, 'mo_end': False, 'purchase_orders': []}

        end_dt = self._get_replan_target_end_dt()
        current_end_day = end_dt.date()
        lines = []
        for wo in workorders.sorted(lambda w: (w.operation_id.sequence if w.operation_id else 0, w.id), reverse=True):
            wc = wo.workcenter_id
            cal = wc.resource_calendar_id or self.env.company.resource_calendar_id
            ...
            effective_minutes = (wo.duration_expected or 0.0) / max(1, nb_resources)
            end_line_dt = start_dt + timedelta(minutes=effective_minutes)
            lines.append({
                'workorder_id': wo.id,
                'name': wo.name,
                'workcenter': wc.display_name if wc else '',
                'start': fields.Datetime.to_string(start_dt),
                'end': fields.Datetime.to_string(end_line_dt),
                'macro_planned_start': fields.Datetime.to_string(start_dt),
                'nb_resources': nb_resources,
            })
            current_end_day = first_day - timedelta(days=1)
        lines.reverse()
        mo_start = lines[0]['start'] if lines else False
        mo_end = lines[-1]['end'] if lines else False
        purchase_orders = []
        for po in self._collect_related_purchase_orders():
    ...
    def _build_replan_summary_html(self, payload):
        self.ensure_one()
        mo_start = payload.get('mo_start') or ''
        mo_end = payload.get('mo_end') or ''
        html = [f"<p><b>OF :</b> {self.display_name}</p>"]
        html.append(f"<p><b>Nouvelle date de début fabrication :</b> {mo_start}<br/><b>Nouvelle date de fin fabrication :</b> {mo_end}</p>")
        html.append("<p><b>Opérations recalculées :</b></p><ul>")
        for line in payload.get('lines', []):
            html.append(f"<li>{line['name']} - {line['workcenter']} : {line['start']} → {line['end']} ({line['nb_resources']} ressource(s))</li>")
        html.append("</ul>")
        html.append("<p><b>PO liés :</b></p><ul>")
        for po in payload.get('purchase_orders', []):
            html.append(f"<li>{po['name']} - {po['partner']} - livraison prévue {po['date_planned']} - état {po['state']}</li>")
        html.append("</ul>")
        return ''.join(html)
    ...
    def action_open_replan_preview(self):
        ...
        wizard = self.env['mrp.replan.preview.wizard'].create({
            'production_id': self.id,
            'summary_html': self._build_replan_summary_html(payload),
            'preview_json': __import__('json').dumps(payload),
        })
        view = self.env.ref('mrp_replan_workorder.mrp_replan_preview_wizard_form')
        return {
            'type': 'ir.actions.act_window',
            'name': 'Prévisualisation recalcul OF',
    ...
    def action_apply_replan_preview(self, payload):
        self.ensure_one()
        for line in payload.get('lines', []):
            wo = self.env['mrp.workorder'].browse(line['workorder_id'])
            if not wo.exists():
                continue
            start_dt = fields.Datetime.to_datetime(line['start']) if line.get('start') else False
            end_dt = fields.Datetime.to_datetime(line['end']) if line.get('end') else False
            vals = {
                'macro_planned_start': start_dt,
                'x_nb_resources': line.get('nb_resources', 1),
            }
            if 'date_start' in wo._fields:
                vals['date_start'] = start_dt
            if 'date_finished' in wo._fields:
        ...
        mo_start = fields.Datetime.to_datetime(payload.get('mo_start')) if payload.get('mo_start') else False
        mo_end = fields.Datetime.to_datetime(payload.get('mo_end')) if payload.get('mo_end') else False
        write_vals = {}
        if mo_start:
            write_vals['date_start'] = mo_start
        if mo_end:
            write_vals['date_finished'] = mo_end
            write_vals['macro_forced_end'] = mo_end
        if write_vals:
            self.with_context(mail_notrack=True).write(write_vals)
        try:
            self._update_components_picking_dates()
        except Exception:
            _logger.exception('Erreur mise à jour picking composants après recalcul OF %s', self.name)
        return True
