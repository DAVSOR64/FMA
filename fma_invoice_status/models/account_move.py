    ...
    def _update_invoices(self, file_content):
        ...
        for row in rows:
            ...
            if invoice and sign == '+':
                ...
                # Récupérer le mode de paiement depuis le journal de la facture
                payment_method = invoice.journal_id.inbound_payment_method_line_ids[:1].payment_method_id
                if not payment_method:
                    _logger.error("❌ Aucun mode de paiement défini sur le journal %s (facture %s).", invoice.journal_id.name, invoice.name)
                    return
