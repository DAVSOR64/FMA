...
def migrate(cr, version):
    """Replace tax_id → tax_ids in all ir.ui.view records inheriting from
    sale.report_saleorder_document. The field was renamed in Odoo v19 and
    Studio-generated views stored in DB still reference the old name."""
    ...
    for view in views:
        if 'tax_id' not in (view.arch_db or ''):
            continue
        # Only replace standalone tax_id — do not touch tax_ids
