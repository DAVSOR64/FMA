...
class SaleOrder(models.Model):
    ...
    chat_template_id = fields.Many2one(
        "affair.chat.template", string="Modèle de message"
    )
    ...
    so_retard_nouvelle_date = fields.Date(string="Nouvelle date de livraison (retard)")
    so_retard_motif = fields.Char(string="Motif du retard")

    # Nouveau champ ARC pour les mails / communication
    so_arc_ref = fields.Char(string="Référence ARC")
    ...
    so_type_camion_bl = fields.Selection(
        [
            ...
            (
                "Semi-remorque chariot embarqué (650€)",
                "Semi-remorque chariot embarqué (650€)",
            ),
    ...
    so_horaire_ouverture_bl = fields.Float(
        string="Horaire ouverture", widget="float_time"
    )
    so_horaire_fermeture_bl = fields.Float(
        string="Horaire fermeture", widget="float_time"
    )
    ...
    so_date_de_reception_devis = fields.Date(
        string="Demande reçue le :", required=True, default=fields.Date.today()
    )
    ...
    so_date_de_livraison = fields.Date(
        string="Livraison prévue le : ",
        compute="_compute_so_date_de_livraison",
        store=True,
    )
    ...
    so_marge_brute_devis = fields.Monetary(
        string="Marge Brute en € (Devis)",
        compute="_compute_so_marge_brute_devis",
        store=True,
    )
    so_prc_marge_brute_devis = fields.Float(
        string="Marge Brute en % (Devis)",
        compute="_compute_so_prc_marge_brute_devis",
        store=True,
    )
    so_mcv_devis = fields.Monetary(
        string="M.C.V. en € (Devis)", compute="_compute_so_mcv_devis", store=True
    )
    so_prc_mcv_devis = fields.Float(
        string="M.C.V. en % (Devis)", compute="_compute_so_prc_mcv_devis", store=True
    )
    ...
    so_prc_mcv_devis_display = fields.Char(
        compute="_compute_so_prc_mcv_devis_display", string="M.C.V. en % (Devis)"
    )

    @api.depends("so_prc_marge_brute_devis")
    def _compute_so_prc_marge_brute_devis_display(self):
        for record in self:
            marge_brute_arrondie = "{:.1f}".format(
                float_round(record.so_prc_marge_brute_devis, precision_digits=1)
            )
    ...
    def _compute_so_prc_mcv_devis_display(self):
        for record in self:
            mcv_arrondi = "{:.1f}".format(
                float_round(record.so_prc_mcv_devis, precision_digits=1)
            )
    ...
    so_marge_brute_be = fields.Monetary(
        string="Marge Brute en € (B.E.)",
        compute="_compute_so_marge_brute_be",
        store=True,
    )
    so_prc_marge_brute_be = fields.Float(
        string="Marge Brute en % (B.E.)",
        compute="_compute_so_prc_marge_brute_be",
        store=True,
    )
    so_mcv_be = fields.Monetary(
        string="M.C.V. en € (B.E.)", compute="_compute_so_mcv_be", store=True
    )
    so_prc_mcv_be = fields.Float(
        string="M.C.V. en % (B.E.)", compute="_compute_so_prc_mcv_be", store=True
    )
    ...
    so_prc_mcv_be_display = fields.Char(
        compute="_compute_so_prc_mcv_be_display", string="M.C.V. en % (B.E.)"
    )

    @api.depends("so_prc_marge_brute_be")
    def _compute_so_prc_marge_brute_be_display(self):
        for record in self:
            marge_brute_arrondie_be = "{:.1f}".format(
                float_round(record.so_prc_marge_brute_be, precision_digits=1)
            )
    ...
    def _compute_so_prc_mcv_be_display(self):
        for record in self:
            mcv_arrondi_be = "{:.1f}".format(
                float_round(record.so_prc_mcv_be, precision_digits=1)
            )
    ...
    so_marge_brute_reel = fields.Monetary(
        string="Marge Brute en € (Réel)",
        compute="_compute_so_marge_brute_reel",
        store=True,
    )
    so_prc_marge_brute_reel = fields.Float(
        string="Marge Brute en % (Réel)",
        compute="_compute_so_prc_marge_brute_reel",
        store=True,
    )
    so_mcv_reel = fields.Monetary(
        string="M.C.V. en € (Réel)", compute="_compute_so_mcv_reel", store=True
    )
    so_prc_mcv_reel = fields.Float(
        string="M.C.V. en % (Réel)", compute="_compute_so_prc_mcv_reel", store=True
    )
    ...
    so_prc_mcv_reel_display = fields.Char(
        compute="_compute_so_prc_mcv_reel_display", string="M.C.V. en % (Réel)"
    )

    @api.depends("so_prc_marge_brute_reel")
    def _compute_so_prc_marge_brute_reel_display(self):
        for record in self:
            marge_brute_arrondie_reel = "{:.1f}".format(
                float_round(record.so_prc_marge_brute_reel, precision_digits=1)
            )
    ...
    def _compute_so_prc_mcv_reel_display(self):
        for record in self:
            mcv_arrondi_reel = "{:.1f}".format(
                float_round(record.so_prc_mcv_reel, precision_digits=1)
            )
    ...
    def _compute_so_date_de_livraison(self):
        for order in self:
            if order.so_date_bpe and order.so_delai_confirme_en_semaine:
                ...
                order.so_date_de_livraison_prevu = order.so_date_bpe + timedelta(
                    weeks=order.so_delai_confirme_en_semaine
                )
    ...
    @api.depends(
        "so_mtt_facturer_devis", "so_achat_vitrage_devis", "so_achat_matiere_devis"
    )
    def _compute_so_marge_brute_devis(self):
        for order in self:
            order.so_marge_brute_devis = (
                order.so_mtt_facturer_devis
                - order.so_achat_vitrage_devis
                - order.so_achat_matiere_devis
            )
    ...
    def _compute_so_marge_brute_be(self):
        for order in self:
            order.so_marge_brute_be = (
                order.so_mtt_facturer_be
                - order.so_achat_vitrage_be
                - order.so_achat_matiere_be
            )
    ...
    @api.depends(
        "so_mtt_facturer_reel", "so_achat_vitrage_reel", "so_achat_matiere_reel"
    )
    def _compute_so_marge_brute_reel(self):
        for order in self:
            order.so_marge_brute_reel = (
                order.so_mtt_facturer_reel
                - order.so_achat_vitrage_reel
                - order.so_achat_matiere_reel
            )
    ...
    def _compute_so_prc_marge_brute_devis(self):
        for order in self:
            if order.so_mtt_facturer_devis:
                order.so_prc_marge_brute_devis = (
                    order.so_marge_brute_devis / order.so_mtt_facturer_devis
                ) * 100
            else:
    ...
    def _compute_so_prc_mcv_devis(self):
        for order in self:
            if order.so_mtt_facturer_devis:
                order.so_prc_mcv_devis = (
                    order.so_mcv_devis / order.so_mtt_facturer_devis
                ) * 100
            else:
    ...
    def _compute_so_prc_marge_brute_be(self):
        for order in self:
            if order.so_mtt_facturer_be:
                order.so_prc_marge_brute_be = (
                    order.so_marge_brute_be / order.so_mtt_facturer_be
                ) * 100
            else:
    ...
    def _compute_so_prc_marge_brute_reel(self):
        for order in self:
            if order.so_mtt_facturer_reel:
                order.so_prc_marge_brute_reel = (
                    order.so_marge_brute_reel / order.so_mtt_facturer_reel
                ) * 100
            else:
    ...
    def _compute_so_prc_mcv_reel(self):
        for order in self:
            if order.so_mtt_facturer_reel:
                order.so_prc_mcv_reel = (
                    order.so_mcv_reel / order.so_mtt_facturer_reel
                ) * 100
            else:
